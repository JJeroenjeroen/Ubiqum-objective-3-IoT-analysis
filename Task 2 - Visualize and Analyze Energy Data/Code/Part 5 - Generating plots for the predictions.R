...
#plot general trend
ggplot(forecast_GAP) +
  geom_line(aes(x = ds, y = trend*60), colour = "#1380A1", size = 1.2) + 
  geom_hline(yintercept = 60, size = 1, colour="#333333") +
  bbc_style() +
...
ggplot(forecast_GAP) +
  geom_point(aes(x = ds, y = y*60), colour = "black", size = .1) +
  geom_line(aes(x =ds, y = ((trend+yearly)*60)), colour = "red", size = 1) + 
...
ggplot(forecast_GAP) +
  geom_line(aes(x = ds, y = (yhat_upper)*60), colour = "grey", size = 0.1, alpha = 0.5) + 
  geom_line(aes(x = ds, y = (yhat_lower)*60), colour = "grey", size = 1, alpha = 0.5) + 
  geom_ribbon(aes(x = ds, ymin = yhat_lower*60,ymax = yhat_upper*60), fill="grey", alpha="0.3") +
  geom_point(aes(x = ds, y = y*60), colour = "black", size = 0.1) +
  geom_line(aes(x = ds, y = (yhat)*60), colour = "#FAAB18", size = 0.1) + 
  geom_hline(yintercept = 0, size = 1, colour="#333333") +
  bbc_style() +
  labs(title="Prophet Forecast",
       subtitle = "Average kWh From 2007-2012")
...
ggplot(weekly_values) +
  ...
  geom_hline(yintercept = -.15, size = 1, colour="#333333") +
  bbc_style() +
