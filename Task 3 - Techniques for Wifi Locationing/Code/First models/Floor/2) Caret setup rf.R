...
#train rf model 
set.seed(123)
rf_Fit1 <- train(x = wifi_train_xvalues, 
                y = wifi_train_yvalues$FLOOR,
                method = train_method,
                metric = train_metric,
...
Prediction_rf <- predict(rf_Fit1, newdata = wifi_test_xvalues)
postResample(Prediction_rf, wifi_test_yvalues$FLOOR)
...
confusionMatrix(data = Prediction_rf, wifi_test_yvalues$FLOOR)
